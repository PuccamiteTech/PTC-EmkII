...
rem The extraction script fails when given an absolute path or a path with spaces.
rem Instead of tweaking it nonstop, I'm keeping stuff simple.
set "ARCHIVE_FILE=rom.nds"
...
copy /y "%LIB_DIR%*" && ^
cd /d "%RES_DIR%" && ^
call "%EXTRACTOR_FILE%" "%ARCHIVE_FILE%" && ^
cd /d "%PROJECT_DIR%"